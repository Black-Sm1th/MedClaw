      ...
      try {
        if (window.parent !== window) {
          ...
          const base64 = await new Promise<string>((resolve, reject) => {
            reader.onload = () => {
              ...
              if (separator < 0) reject(new Error('PPTX 导出数据无效'))
              else resolve(dataUrl.slice(separator + 1))
            }
            reader.onerror = () => reject(reader.error || new Error('PPTX 导出读取失败'))
            reader.readAsDataURL(output)
          })
          window.parent.postMessage({ type: 'pptx-exported', base64 }, '*')
        }
        else {
          await pptx.writeFile({ fileName: `pptist.pptx` })
        }
      }
      catch (error) {
        message.error('导出失败')
        if (window.parent !== window) {
          window.parent.postMessage({
            type: 'pptx-error',
            message: error instanceof Error ? error.message : 'PPTX 导出失败',
          }, '*')
        }
      }
      finally {
