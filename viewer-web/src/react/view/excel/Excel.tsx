...
function ExcelViewer() {
    ...
    useEffect(() => {
        const hostWindow = window as Window & { __officeViewerRequestSaveAs?: () => boolean };
        const requestSaveAs = () => {
            handleSaveAs();
            return true;
        };
        ...
        return () => {
            if (hostWindow.__officeViewerRequestSaveAs === requestSaveAs) {
                delete hostWindow.__officeViewerRequestSaveAs;
            }
        };
    }, [handleSaveAs]);
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            const saved = await export_xlsx(spreadSheet, extRef.current, csvEncoding, undefined, csvDelimiter);
            if (saved) spreadSheet.setSaveEnabled(false);
        } catch (error) {
            console.error(`Failed to save Excel file: ${(error as Error).message}`);
            message.error({ duration: 3, content: `${t('button.save')}: ${(error as Error).message}` });
        }
    }, [modal, handleSaveAs, message]);
    ...
    const confirmSaveAs = useCallback(async (fmt: string) => {
        ...
        try {
            const saved = await exportSaveAs(spreadSheet, fmt, csvEncodingRef.current, csvDelimiterRef.current);
            if (saved && !readOnlyRef.current) {
                spreadSheet.setSaveEnabled(false);
            }
        } catch (error) {
            console.error(`Failed to save Excel file: ${(error as Error).message}`);
            message.error({ duration: 3, content: `${t('button.saveAs')}: ${(error as Error).message}` });
        }
    }, [message]);
    ...
    useEffect(() => {
        ...
        handler.on("open", (payload) => {
            ...
            message.success({ duration: 1.8, content: t('viewer.saveSuccess'), className: 'excel-save-success-message' });
        }).emit("init")
