...
async function emitSave(buffer: Uint8Array, options?: ExportOptions) {
    ...
    if (options?.saveAs) {
        const response = await handler.emitAsync('saveAs', { content, ext: options.saveAsExt ?? 'xlsx' });
        return !response.events?.some(event => event.type === 'saveCanceled');
    }
    await handler.emitAsync('save', content);
    return true;
}
...
async function exportWithExcelJs(sheets: SheetData[], options?: ExportOptions) {
    ...
    return await emitSave(patched, options);
}
...
async function exportWithSheetJs(sheets: SheetData[], bookType: XLSX.BookType) {
    ...
    await handler.emitAsync('save', [...new Uint8Array(buffer)]);
    return true;
}
...
export async function exportSaveAs(
    ...
    if (ext === 'xlsx' || ext === 'xlsm') {
        return await exportWithExcelJs(sheets, { saveAs: true, saveAsExt: ext });
    }
    if (ext === 'xls' || ext === 'ods') {
        ...
        const response = await handler.emitAsync('saveAs', { content: [...new Uint8Array(buf)], ext });
        return !response.events?.some(event => event.type === 'saveCanceled');
    }
    if (ext === 'csv' || ext === 'tsv') {
        ...
        const response = await handler.emitAsync('saveAs', { content: [...bytes], ext });
        return !response.events?.some(event => event.type === 'saveCanceled');
    }
    return false;
}
...
export async function export_xlsx(
    ...
    if (ext === 'xlsx' || ext === 'xlsm' || options?.saveAs) {
        return await exportWithExcelJs(sheets, options?.saveAs ? { saveAs: true } : undefined);
    }
    if (ext === 'xls' || ext === 'ods') {
        return await exportWithSheetJs(sheets, ext);
    }
    if (ext === 'csv' || ext === 'tsv') {
        const fs = ext === 'tsv' ? '\t' : csvDelimiter;
        const csvContent = XLSX.utils.sheet_to_csv(dataToSheetJs(sheets[0]), { FS: fs });
