...
QString SessionReader::sessionsDir() const
{
    return m_sessionsDir;
}

QVariantList SessionReader::sessionList() const
{
...
QVariantMap SessionReader::quickParseSummary(const QString &filePath)
{
    ...
    while (!file.atEnd()) {
        ...
        if (type == QLatin1String("session")) {
            info[QStringLiteral("sessionId")] = obj.value(QStringLiteral("id")).toString();
            info[QStringLiteral("version")] = obj.value(QStringLiteral("version")).toInt();
            info[QStringLiteral("createdAt")] = obj.value(QStringLiteral("timestamp")).toString();
            info[QStringLiteral("cwd")] = obj.value(QStringLiteral("cwd")).toString();
        }
        // 模型变更记录
        else if (type == QLatin1String("model_change")) {
            info[QStringLiteral("modelProvider")] = obj.value(QStringLiteral("provider")).toString();
            info[QStringLiteral("modelId")] = obj.value(QStringLiteral("modelId")).toString();
        }
        // 思考级别
        else if (type == QLatin1String("thinking_level_change")) {
            info[QStringLiteral("thinkingLevel")] = obj.value(QStringLiteral("thinkingLevel"))
                                                        .toString();
        }
        // 消息：统计并提取首条用户消息
                ...
                if (firstUserMsg.isEmpty()) {
                    ...
                    for (const QJsonValue &cv : arr) {
                        ...
                        if (co.value(QStringLiteral("type")).toString() == QLatin1String("text")) {
                            ...
                            if (bracket >= 0 && bracket < 40 && t.contains(QStringLiteral("GMT")))
...
void SessionReader::scanSessions()
{
    ...
    QString activeSessionId;
    {
        ...
        if (sjFile.open(QIODevice::ReadOnly)) {
            const QJsonObject root = QJsonDocument::fromJson(sjFile.readAll()).object();
            sjFile.close();
            for (auto it = root.begin(); it != root.end(); ++it) {
                activeSessionId = it.value().toObject().value(QStringLiteral("sessionId")).toString();
    ...
    for (const QString &fn : allFiles) {
        ...
        const QString sessionId = summary.value(QStringLiteral("sessionId")).toString();
        ...
        // ── 构建显示名称 ──
        QString displayName;
        const QString preview = summary.value(QStringLiteral("firstUserMsg")).toString();
        const int msgCnt = summary.value(QStringLiteral("messageCount")).toInt();
        const QString model = summary.value(QStringLiteral("modelId")).toString();
        ...
        entry[QStringLiteral("timestamp")] = summary.value(QStringLiteral("createdAt"));
        entry[QStringLiteral("displayName")] = displayName;
        entry[QStringLiteral("preview")] = preview;
        entry[QStringLiteral("messageCount")] = msgCnt;
        entry[QStringLiteral("modelProvider")] = summary.value(QStringLiteral("modelProvider"));
        entry[QStringLiteral("modelId")] = model;
        entry[QStringLiteral("resetTime")] = resetTime;
        entry[QStringLiteral("userMsgCount")] = summary.value(QStringLiteral("userMsgCount"));
        entry[QStringLiteral("assistantMsgCount")] = summary.value(
            QStringLiteral("assistantMsgCount"));
        entry[QStringLiteral("toolCallCount")] = summary.value(QStringLiteral("toolCallCount"));
        entry[QStringLiteral("cwd")] = summary.value(QStringLiteral("cwd"));
        entry[QStringLiteral("thinkingLevel")] = summary.value(QStringLiteral("thinkingLevel"));
...
QVariantList SessionReader::readSessionMessages(const QString &filePath)
{
    ...
    while (!file.atEnd()) {
        ...
        for (const QJsonValue &cv : contentArr) {
            ...
            if (ctype == QLatin1String("text")) {
                ...
                if (role == QLatin1String("user")) {
                    const QRegularExpression tsRegex(QStringLiteral("^\\[.*?GMT[+-]\\d+\\]\\s*"));
    ...
    file.close();
    qDebug() << "[SessionReader] parsed" << messages.count() << "entries from"
...
QVariantList SessionReader::parseResponseFile(const QString &filePath)
{
    ...
    while (searchFrom < raw.size()) {
        ...
        QJsonArray arr = obj.value(QStringLiteral("payload"))
                             .toObject()
        ...
        if (!arr.isEmpty()) {
            qDebug() << "[SessionReader] found messages block with" << arr.count() << "messages";
    ...
    if (messagesArr.isEmpty()) {
        ...
        if (doc.isObject()) {
            messagesArr = doc.object()
                              ...
                              .toArray();
            if (messagesArr.isEmpty())
                messagesArr = doc.object().value(QStringLiteral("messages")).toArray();
        }
    }

    if (messagesArr.isEmpty()) {
        qWarning() << "[SessionReader] parseResponseFile: no messages found in" << filePath;
        return result;
    }

    qDebug() << "[SessionReader] parseResponseFile: found" << messagesArr.count() << "raw messages";
    ...
    for (const QJsonValue &v : messagesArr) {
        ...
        const qint64 tsMs = static_cast<qint64>(m.value(QStringLiteral("timestamp")).toDouble());
        const QString ts = QDateTime::fromMSecsSinceEpoch(tsMs).toString(
    ...
    qDebug() << "[SessionReader] parseResponseFile: parsed" << result.count() << "display entries";
    return result;
}
...
QVariantList SessionReader::readAgentList()
{
    ...
    for (const QJsonValue &v : list) {
        ...
        entry[QStringLiteral("sessionKey")] = QStringLiteral("agent:%1:main").arg(id);
        entry[QStringLiteral("isDefault")] = a.value(QStringLiteral("default")).toBool(false);
        entry[QStringLiteral("workspace")] = a.value(QStringLiteral("workspace")).toString();
        entry[QStringLiteral("agentDir")] = a.value(QStringLiteral("agentDir")).toString();
